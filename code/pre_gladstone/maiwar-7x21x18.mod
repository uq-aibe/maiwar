...
#
param UInf 'infimum of interval for uniform dbn' default 0.09 in [0, .5);
param USup 'supremum of interval for uniform dbn' default 0.91 in (.5, 1];
...
var inv_sec_CD 'Cobb-Douglas investment aggregate (across sectors)'
  {r in Regions, j in Sectors, t in LookForward}
  = prod{i in Sectors} inv[r, i, j, t] ** (SHR_SEC_INV[r, i, j] * SCALE_INV);
...
display A;
for {i in Sectors}{
  let A[i] := 5600e-2; #* (card(Sectors) / 20) ** (1 - 20e-2);
  let DELTA[i] := 08e-2;
...
#-----------set and solve the plan for start time s
  ...
  let InstanceName := ("maiwar"
    ...
    & "x" & card(LookForward) & "x" & card(PathTimes));
