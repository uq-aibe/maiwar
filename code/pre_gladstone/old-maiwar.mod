...
param JAC_ID 'observed jacobi identity' {Sectors, Sectors, PathTimes}
...
#-----------save actual path values of variables to parameter
      ...
      for {j in Sectors}{
        ...
        let JAC_ID[i, j, s - 1] := max{ii in Sectors, jj in Sectors}(
          sum{rr in Regions}(
            INV[rr, i, j, s] / INV_SHR[rr, i , j]
            - (INV[rr, i, ii, s - 1] / INV_SHR[rr, i, ii])
              / (INV[rr, jj, ii, s - 1] / INV_SHR[rr, jj, ii])
              * (INV[rr, jj, j, s - 1] / INV_SHR[rr, jj, j])
          )
        );
      };
...
#-----------set and solve the plan for start time s
    ...
    display s, _ampl_elapsed_time, _total_solve_time,
      E_OUTPUT, CON, INV_SUM, ADJ_COST_KAP, MKT_CLR, LAB, KAP, JAC_ID;
  };
};
