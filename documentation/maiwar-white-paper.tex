...
model in detail for an with certainty.
\section{A canonical example without uncertainty}
Under certainty the solution is a known path through time.
...
adjusting capital:
\begin{equation}
   \textup{adj}_{r, i}(x, x^{\mplus}) \mrpropto  
   \frac{(x^{\mplus} - x)^2}{x}.
\end{equation}
