...
In \eqref{eq-kapital}, 
$\textup{inv}_{r, j, n + t}$ is an aggregator over the fixed capital formation
vector $(\mathbf{sav}_{r, ij, n + t}: i \in J)$.
...
\begin{equation}\label{eq-ces-inv}
  \textup{inv}_{r, j, n + t} = A_{r, j}^{\textup{inv}}
    \cdot \left(\sum_{i \, \in \, J}
      \sigma_{i j} ^ {\textup{inv}} \cdot \mathbf{sav}_{r, i j,\, n + t}
        ^ {\,\rho_{j} ^ {\textup{inv}}}
...
\begin{equation}
  ...
  - \mathbf{con}_{r, i}
  - \sum_{j \,\in\, J}
      \left\{\mathbf{sav}_{r, ij} + \mathbf{med}_{r, ij}\right\}
  - \textup{adj}_{r, i}
\right\}.
\end{equation}
