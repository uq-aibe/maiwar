...
param EoS_KAP 'elasticity of substitution for kapital' default 2e-1 > 0; 
...
param SCALE 'economies of scale: ensures concavity' default 98e-2 in (0, 1);
/*=============================================================================
Computed parameters
=============================================================================*/
...
param A 'productivity trend' {i in Sectors}
  default 9e+2 * (1 - (1 - DELTA[i]) * BETA) / (ALPHA[i] * BETA) >= 0;
...
var E_output 'current intermediate variable for output'
  {r in Regions, i in Sectors, t in LookForward}
    = E_output_CES[r, i, t] * 1e+0;
var inv_sec 'current intermediate variable for aggregated investment'
  {r in Regions, j in Sectors, t in LookForward}
    #= __INVSEC__[r, j, t];
    = inv_sec_CES[r, j, t] * 245e-2; # for 4x10 set to 345e-2 when EoS is 3;
