...
#-----------lower values of LSup are more myopic; longer satisfy Euler Eqn.
...
var inv 'investment flows'
  {r in Regions, i in Sectors, j in Sectors, LookForward, PathTimes}
...
var inv_sum 'sum over destinations for saving'
  {r in Regions, i in Sectors, s in PathTimes}
    = sum{j in Sectors} inv[r, i, j, LInf, s];
#var kap_growth {r in Regions, i in Sectors, s in PathTimes}
#  = (kap[r, i, LSup + 1] - KAP[r, i, LSup]) / KAP[r, i, s];
...
#-----------display step values
  ...
  inv_sec["SEQ", "PbSc", LInf, s], kap["SEQ", "PbSc", LInf],
  lab["SEQ", "PbSc", LInf], kap_transition["SEQ", "PbSc", LInf, s],
  market_clearing["PbSc", LInf, s];
