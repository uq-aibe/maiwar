...
param OInf 'infimum of interval for basic variables' default 1e-4;
param OSup 'supremum of interval for basic variables' default 1e+4;
#
...
param CON 'observed consumption' {Regions, Sectors, PathTimes} default 1e+0 
  in (OInf, OSup); # Conopt suggest rescaling if not in this range
param INV_SEC 'observed investment' {Regions, Sectors, PathTimes} default 1e+0 
  in (OInf, OSup); # Conopt suggest rescaling if not in this range
param INV_SUM 'observed total investment' {Regions, Sectors, PathTimes}
  default 1e+0 in (OInf, OSup); # Conopt suggest rescaling if not in this range
param LAB 'observed labour' {Regions, Sectors, PathTimes} default 1e+0 
  in (OInf, OSup); # Conopt suggest rescaling if not in this range
...
var tail_val_CD 'Cobb-Douglas continuation value from time LSup + LInf onwards'
  {s in PathTimes}
  = sum{r in Regions} prod{i in Sectors}
