...
\begin{equation}\label{eq-kapital}
   \mathbf{kap}_{r, i, n + t + 1} =
     (1 - \delta_{r, i}) \cdot \mathbf{kap}_{r, i, n + t}
       + \textup{inv}_{r, i, n + t}.
\end{equation}
...
Suppressing regional subscripts, for every $j$ in $J$ and $n + t$ we have
...
\end{equation}
As usual, $\rho ^ \textup{inv}
  = \frac{\epsilon ^ \textup{inv} - 1}{\epsilon ^ \textup{inv}}$
is a parameter that captures the role of the elasticity of substitution
$\epsilon ^ \textup{inv} \approx 0.1$ across sectors of production.
...
sufficient condition for strict concavity of the function
$\left\{\mathbf{sav}_{ij, n + t}: i \in J\right\}
\mapsto \textup{inv}_{j, n + t}$ is that $0< \kappa_{j}^\textup{inv} < 1$.
...
with its corresponding set of parameters. That is,
\begin{equation}\label{eq-ces-int}
  \textup{int}_{r, j, n + t} = A_{r, j, n + t} ^ \textup{int}
    \cdot \textup{CES}_{r, j} ^ \textup{\,int}(
      \mathbf{med}_{r, ij, n + t}: i \in J
      )
\end{equation}
where, for each $r$, $n$ and $t$, $\{\textbf{med}_{r,ij,n + t} : i, j \in J\}$
is a standard matrix of intermediate (input-output) flows prior to aggregation.
...
\begin{equation}
	\textup{out}_{r, i, n + t}
    = A_{r, i, n + t} ^ \textup{out} \cdot \textup{CES}_{r, i}
      ^ \textup{\,out}(
...
\end{equation}
%\begin{equation}
%  \textup{CES}_{r, i, n + t} ^ {\textup{out}} (x, y, z)
%	  = \left(
...
\begin{equation}
   ...
   \sum_{i \,\in\, J} \gamma_{r, i} \cdot \mathbf{con}_{r, i, n + t} ^ {\,\rho}
   \right) ^ {\frac{\kappa}{\rho}}
   - \sum_{i \,\in\, J} \lambda_{r, i} \cdot \mathbf{lab}_{r, i, n + t}^{2}
\end{equation}
