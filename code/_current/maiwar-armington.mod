...
-----------------------------------------------------------------------------*/
var kap 'capital stocks (dynamic: defined on LookForwardClosure)'
  {r in Regions, j in Sectors, LookForwardClosure}
  in [VInf, VSup] default KAP[r, j, PInf]; 
/*-----------------------------------------------------------------------------
#-----------Armington variables
-----------------------------------------------------------------------------*/
var prc_dom 'domestic price / lagrange multiplier'
  {j in Sectors, t in LookForward}
  = SCALE_CON * SHR_CON_CES['SEQ', j]
    * (A_CON * con_sec_CES['SEQ', t] / con['SEQ', j, t]) ** (1 - RHO_CON)
var ypo 'imports'
  {r in Regions, i in Sectors, j in Sectors, t in LookForward} in [VInf, VSup]
...
var med_sec_CES 'Const. Elast. Subst. intermediate aggregate (across sectors)'
  {r in Regions, j in Sectors, t in LookForward}
  = (sum{i in Sectors} SHR_MED_CES[r, i, j] * med[r, i, j, t] ** RHO_MED)
...
subject to trn_kap 'equation for the accumulation of kapital'
  {r in Regions, j in Sectors, t in LookForward}:
    kap[r, j, t + 1] - (1 - DELTA[j]) * kap[r, j, t] - inv_sec[r, j, t] = 0;
subject to mkt_clr 'market clearing for each sector and time'
...
#-----------display step values
  ...
  lab["SEQ", "PbSc", LInf], trn_kap["SEQ", "PbSc", LInf],
  mkt_clr["PbSc", LInf];
...
#-----------save actual path values of variables to parameter
    ...
    let DUAL_KAP[r, i, s] := trn_kap[r, i, LInf]
  };
...
#-----------save actual path values of market clearing to parameter
    ...
    let DUAL_MKT_CLR[i, s] := mkt_clr[i, LInf];
  };
...
#-----------Euler integrand for CES production
    let EULER_INTEGRAND[r, i, s] := DUAL_KAP[r, i, s] * (1 - DELTA[i]) 
      + DUAL_MKT_CLR[i, s] * (
        SCALE_OUT * SHR_KAP_OUT_CES[i] * LAB_EXT[r, i, s] ** RHO_OUT
          * A[i] ** (RHO_OUT / SCALE_OUT)
